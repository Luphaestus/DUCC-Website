...
describe('api/TagsAPI', () => {
    ...
    describe('CRUD Operations', () => {
        test('GET /api/tags - Authorized public access', async () => {
            ...
            expect(JSON.parse(res.body).data.some(t => t.name === 'T1')).toBe(true);
        });
        ...
        test('POST /api/tags - RBAC enforcement', async () => {
            ...
            // Guest should fail
            const resGuest = await world.request.post('/api/tags', tagData);
            expect(resGuest.statusCode).toBe(401);

            // Standard user should fail
            const resUser = await world.as('user1').post('/api/tags', tagData);
            expect(resUser.statusCode).toBe(403);

            // Admin should succeed
            const resAdmin = await world.as('admin').post('/api/tags', tagData);
            expect(resAdmin.statusCode).toBe(201);
            expect(JSON.parse(resAdmin.body).data).toHaveProperty('id');
        });
        ...
        test('PUT /api/tags/:id and DELETE /api/tags/:id lifecycle', async () => {
            ...
            // Update
            const resUpdate = await world.as('admin').put(`/api/tags/${tagId}`, { name: 'Updated' });
        ...
        test('Tag manager can update their managed tag', async () => {
            ...
            const res = await world.as('user1').put(`/api/tags/${tagId}`, {
                name: 'M1-Updated',
                image_id: fileId
            });
            expect(res.statusCode).toBe(200);
        ...
        test('Standard user cannot update tag they do not manage', async () => {
            ...
            const res = await world.as('user1').put(`/api/tags/${tagId}`, { name: 'Hack' });
    ...
    describe('Whitelist Management', () => {
        ...
        test('Whitelist lifecycle: Add, Get, Remove', async () => {
            // Add user
            const resAdd = await world.as('admin').post(`/api/tags/${tagId}/whitelist`, { userId: targetUserId });
            expect(resAdd.statusCode).toBe(200);

            // Verify in list
            const resGet = await world.as('admin').get(`/api/tags/${tagId}/whitelist`);
            expect(JSON.parse(resGet.body).data.some(u => u.id === targetUserId)).toBe(true);
            ...
            expect(JSON.parse(resGet2.body).data.some(u => u.id === targetUserId)).toBe(false);
        });
    ...
    describe('Manager Scoping Management', () => {
        ...
        test('Assign and revoke direct tag management scope', async () => {
            // Assign
            await world.as('admin').post(`/api/tags/${tagId}/managers`, { userId: targetUserId });
    ...
    describe('User-Specific Tag Lookups', () => {
        ...
        test('Current user can see their own whitelisted tags', async () => {
            ...
            expect(res.statusCode).toBe(200);
            expect(JSON.parse(res.body).some(t => t.name === 'T1')).toBe(true);
        });
