...
import { switchView, ViewChangedEvent } from '@/utils/view';
import { apiRequest } from '@/utils/api';
import { LoginEvent } from '../pages/login.js';
import { BalanceChangedEvent, FirstNameChangedEvent, MembershipChangedEvent } from '@/utils/events/events';
 ...
 */
async function updateBalanceInNav(): Promise<void> {
 ...
 * 
 * @param {NavEntry} entry - Navigation item configuration.
 * @returns {HTMLElement} - The list item element.
 */
function create_item(entry: NavEntry): HTMLElement {
 ...
 * 
 * @param {any} data - Auth status data from /api/auth/status.
 */
async function updateNavOnLoginState(data: any): Promise<void> {
    ...
    authIds.forEach(id => {
        ...
        if (el && el.parentElement) el.parentElement.classList.toggle('hidden', !isLoggedIn);
    });
    ...
    if (isLoggedIn) {
        try {
            ...
            memberIds.forEach(id => {
                const el = document.getElementById(id);
                if (el && el.parentElement) el.parentElement.classList.toggle('hidden', !isMember);
            });
 ...
 */
let toggleMobileMenu: (forceState?: boolean) => void = () => { };
