...
export class TabNav {
    ...
    /**
     * @param {TabNavProps} props
     */
    constructor({ id, tabs, activeKey, classes = '' }: TabNavProps) {
        this.id = id;
        this.tabs = tabs;
     ...
     */
    init(): void {
        const element = document.getElementById(this.id) as HTMLElement & { _syncToggleGroup?: () => void };
        if (!element) return;
        ...
        const bg = element.querySelector('.toggle-bg') as HTMLElement;
        if (!bg) return;

        const sync = (silent = false) => {
            const active = (element.querySelector('.tab-btn.active') || element.querySelector('button.active')) as HTMLElement;
            if (active) {
                ...
                if (element.id) {
                    (window as any).__lastTogglePositions = (window as any).__lastTogglePositions || {};
                    (window as any).__lastTogglePositions[element.id] = pos;
                }
        ...
        const savedPos = element.id ? ((window as any).__lastTogglePositions && (window as any).__lastTogglePositions[element.id]) : null;
     ...
     */
    static initElement(element: HTMLElement): void {
        if (!element) return;
