...
export default function ResendVerificationPage() {
    ...
    const handleSubmit = async (e: Event) => {
        ...
        try {
            await apiRequest('POST', '/api/auth/resend-verification', { email: fullEmail });
            navigate(`/email-sent?type=signup&email=${encodeURIComponent(fullEmail)}`);
        } catch (error: any) {
            triggerError(['email']);
            notify('Error', error.message || 'Failed to resend verification link.', 'error');
        }
    };

    return (
        <div id="resend-verification-view" class="auth-page-wrapper">
            <div class="auth-card">
                <div class="center-text">
                    <h2>
                        Resend Verification
                    </h2>
                    <p class="auth-subtitle">Enter your email and we'll resend you a link to verify your account.</p>
                </div>

                <form onSubmit={handleSubmit}>
                    <div class="modern-form-group">
                        <label for="resend-email">Durham Email Address</label>
                ...
                <div class="auth-footer">
                    <p>Already verified? <a onClick={() => navigate('/login')}>Sign in</a></p>
                    <p style="margin-top: 0.5rem; font-size: 0.85em; opacity: 0.8;">
                        Looking for <a onClick={() => navigate('/reset-password')}>password reset</a>?
                    </p>
                </div>
            </div>
        </div>
