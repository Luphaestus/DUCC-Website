...
import { CURRENCY_POUND_SVG, TRIP_SVG, WALLET_SVG } from '@/utils/icons';
...
async function renderSettlement(container: HTMLElement, eventId: string): Promise<void> {
    try {
        ...
        const costItems: any[] = [
            ...(summary.trips || []).map((t: any) => ({
                ...
                contributions: t.drivers.reduce((acc: any, d: any) => { acc[d.user_id] = d.reimbursement; return acc; }, {})
            })),
        ...
        const tripTablesHtml = (summary.trips || []).map((trip: any) => {
            const rows = trip.drivers.map((d: any) => `
                <tr>
        ...
        const expenseTablesHtml = (summary.expenses || []).map((e: any) => `
            <div class="mb-6">
        ...
        const matrixRowsHtml = (summary.breakdown || []).map((row: any) => {
        ...
        const generateSettlementCSV = () => {
            ...
            (summary.trips || []).forEach((trip: any) => {
                ...
                trip.drivers.forEach((d: any) => csvData.push([d.name, d.miles, d.reimbursement.toFixed(2)]));
                csvData.push([]);
            });
            ...
            (summary.breakdown as any[]).forEach(row => {
        ...
        const downloadBtn = container.querySelector('#download-settlement-csv-btn') as HTMLElement | null;
        if (downloadBtn) downloadBtn.onclick = generateSettlementCSV;
...
ViewChangedEvent.subscribe(({ viewId, path }: any) => {
