...
import { ConditionalRadio, initConditionalRadio } from '@/widgets/conditional_radio';
...
const clearError = (el: HTMLElement | null) => { if (el) el.removeAttribute('aria-invalid'); };
const markError = (el: HTMLElement | null) => { if (el) el.ariaInvalid = 'true'; };
function getRadioBoolean(yesId: string, noId: string) {
    if ((getEl(yesId) as HTMLInputElement)?.checked) return true;
    if ((getEl(noId) as HTMLInputElement)?.checked) return false;
    return null;
}
function setRadioBoolean(yesId: string, noId: string, value: boolean | null) {
    if (value === true) (getEl(yesId) as HTMLInputElement).checked = true;
    else if (value === false) (getEl(noId) as HTMLInputElement).checked = true;
}
...
function initialiseInteractivity() {
    const inputs: NodeListOf<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement> = document.querySelectorAll(`#${DOM_IDS.container} input, #${DOM_IDS.container} select, #${DOM_IDS.container} textarea`);
...
function collectFormData() {
    ...
    SIMPLE_FIELDS.forEach(key => {
        ...
        if (el) payload[key] = (el as HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement).value;
    });

    CHECKBOX_FIELDS.forEach(key => {
        const el = getEl(DOM_IDS[key]);
        if (el) payload[key] = (el as HTMLInputElement).checked;
    });
 ...
 */
function applyFormData(data: Record<string, any>) {
    if (!data) return;

    SIMPLE_FIELDS.forEach(key => {
        const el = getEl(DOM_IDS[key]);
        if (el && data[key] !== null) (el as HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement).value = data[key];
    });

    CHECKBOX_FIELDS.forEach(key => {
        const el = getEl(DOM_IDS[key]);
        if (el) (el as HTMLInputElement).checked = !!data[key];
    });
...
async function renderLegalPage(page: string) {
    ...
    try {
        ...
        if (collegeSelect) {
            (collegeSelect as HTMLSelectElement).innerHTML = '<option value="" disabled selected>Select your college</option>' +
                (collegesRes || []).map((c: any) => `<option value="${c.id}">${c.name}</option>`).join('');
        }
        ...
        if (nameInput && userName.trim() !== 'undefined undefined') {
            (nameInput as HTMLInputElement).value = userName;
...
function setupFormSubmission() {
    ...
    btn.addEventListener('click', async (e) => {
        ...
        try {
            ...
            document.querySelectorAll('[aria-invalid]').forEach(el => clearError(el as HTMLElement));
        } catch (error: any) {
            if (error.errors) {
                let hasError = false;
...
document.addEventListener('DOMContentLoaded', () => {
    setupFormSubmission();
    ViewChangedEvent.subscribe(({ resolvedPath }: any) => renderLegalPage(resolvedPath));
});
