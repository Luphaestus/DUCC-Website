 ...
 * @returns {Promise<any>}
 */
async function getPageData(page: number): Promise<any> {
 ...
 */
function preloadPages(basePage: number): void {
 ...
 * @returns {string}
 */
function getRangeText(startDateStr: string, endDateStr: string): string {
    const formatDate = (d: string | number | Date) => new Date(d).toLocaleDateString('en-UK', { month: 'short', day: 'numeric' });
 ...
 */
function updateUrlParams(push: boolean = true): void {
 ...
 */
async function renderPageContent(page: number, targetElement: HTMLElement): Promise<void> {
    try {
 ...
 */
async function changePage(delta: number | null, animated: boolean = true): Promise<void> {
    ...
    let direction: number = (delta !== null) ? delta : 1;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (e: MouseEvent) => {
        ...
        const btn = target.closest('.nav-btn, .today-btn, .admin-link-btn') as HTMLElement;
        if (btn) {
        ...
        // Navigation
        const todayBtn = target.closest('.today-btn') as HTMLElement;
        if (todayBtn) {
    ...
    EventAttendanceChangedEvent.subscribe(async (data: any) => {
        ...
        try {
            const eventResponse = await apiRequest('GET', `/api/event/${eventId}`);
