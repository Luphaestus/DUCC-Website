...
import { ViewChangedEvent, addRoute, switchView } from '@/utils/view';
import { requireAuth } from '@/utils/auth';
import { getOrdinal, setupNumberInput } from '@/utils/utils';
import { BalanceChangedEvent, MembershipChangedEvent } from '@/utils/events/events';
import { showConfirmModal, showPasswordModal, showChangePasswordModal } from '@/utils/modal';
import { Modal } from '@/widgets/Modal';
import { Sidebar, initSidebar, SidebarItem } from '../widgets/sidebar.js';
...
let sidebarController: { setActive: (tabName: string) => void } | null = null;

// --- Helper Functions ---

function showStatus(title: string, message: string, type: string) {
 ...
 */
function renderMembershipBanner(profile: UserProfile, globals: any) {
    ...
    if (!isMember) {
        ...
        (document.getElementById('join-membership-btn') as HTMLElement).onclick = async () => {
 ...
 */
function renderSwimStats(stats: any) {
 ...
 */
function renderProfilePicture(profile: UserProfile) {
 ...
 */
function renderAvatarControls(profile: UserProfile) {
    ...
    const updatePreset = async (data: any) => {
        try {
    ...
    colorGrid.querySelectorAll('.color-preset').forEach(item => {
        (item as HTMLElement).onclick = () => updatePreset({ color: (item as HTMLElement).dataset.color });
    });

    initialsGrid.querySelectorAll('.initials-preset').forEach(item => {
        (item as HTMLElement).onclick = () => updatePreset({ initials: (item as HTMLElement).dataset.initials });
    });

    fontGrid.querySelectorAll('.font-preset').forEach(item => {
        (item as HTMLElement).onclick = () => updatePreset({ font: (item as HTMLElement).dataset.font });
    });
}
 ...
 */
function renderLegalStatus(profile: UserProfile) {
 ...
 */
function renderSafetyInfo(profile: UserProfile) {
    (document.getElementById('display-first-aid') as HTMLElement).textContent = profile.first_aid_expiry || 'Not Set';
    (document.getElementById('display-emergency') as HTMLElement).textContent = profile.phone_number || 'Not Set';

    (document.getElementById('input-first-aid') as HTMLInputElement).value = profile.first_aid_expiry || '';
    (document.getElementById('input-emergency') as HTMLInputElement).value = profile.phone_number || '';
}
 ...
 */
function renderInstructor(profile: UserProfile) {
    const isInstructor = profile.is_instructor;
    const text = document.getElementById('instructor-status-text') as HTMLElement;
    const btn = document.getElementById('toggle-instructor-btn') as HTMLElement;
 ...
 */
function renderProfileBalance(profile: UserProfile, minMoney: number) {
...
async function renderCars() {
    ...
    try {
        ...
        container.innerHTML = ItemList(cars, (car: Car) => {
        ...
        container.querySelectorAll('[data-edit-car]').forEach(btn => {
            (btn as HTMLElement).onclick = () => {
                const car = cars.find(c => c.id == parseInt((btn as HTMLElement).dataset.editCar!));
        ...
        container.querySelectorAll('[data-delete-car]').forEach(btn => {
            (btn as HTMLElement).onclick = async () => {
                if (await showConfirmModal('Remove Car?', 'Are you sure you want to remove this vehicle?')) {
                    try {
                        await apiRequest('DELETE', `/api/cars/${(btn as HTMLElement).dataset.deleteCar}`);
 ...
 */
function openCarModal(car: Car | null = null) {
    const isEdit = !!car;
    const canManageGlobal = currentUser?.permissions?.includes('car.manage_global');
    ...
    const seatsInput = document.getElementById('car-seats') as HTMLInputElement | null;
    const boatsInput = document.getElementById('car-boats') as HTMLInputElement | null;
    if (seatsInput) setupNumberInput(seatsInput);
    if (boatsInput) setupNumberInput(boatsInput);

    (document.getElementById('car-form') as HTMLFormElement).onsubmit = async (e) => {
        e.preventDefault();
        const data = {
            name: (document.getElementById('car-name') as HTMLInputElement).value,
            seats: (document.getElementById('car-seats') as HTMLInputElement).value,
            boats: (document.getElementById('car-boats') as HTMLInputElement).value,
            isGlobal: (document.getElementById('car-is-global') as HTMLInputElement)?.checked || false
        };
...
async function renderProfileTransactions() {
    ...
    try {
        ...
        container.innerHTML = ItemList(txs, (tx: Transaction) => {
...
async function update2FADisplay() {
    try {
        ...
        const totpBtn = document.getElementById('manage-totp-btn') as HTMLElement;
        const totpStatus = document.getElementById('totp-status') as HTMLElement;
        ...
        (document.getElementById('passkey-count') as HTMLElement).textContent = `${keys.length} key${keys.length !== 1 ? 's' : ''} registered`;
    } catch (err) {
...
async function setupTOTP() {
    try {
        ...
        (document.getElementById('copy-totp-secret') as HTMLElement).onclick = async () => {
            try {
        ...
        (document.getElementById('totp-verify-form') as HTMLFormElement).onsubmit = async (e) => {
            e.preventDefault();
...
async function managePasskeys() {
    ...
    const renderList = (passkeys: any[]) => {
        ...
        listContainer.innerHTML = passkeys.map(k => StandardListItem({
            ...
            actions: `<button class="small-btn icon-only delete" onclick="window.deletePasskey('${k.id}')">${CLOSE_SVG}</button>`
        })).join('');
    ...
    try {
        ...
        (window as any).deletePasskey = async (id: string) => {
        ...
        (document.getElementById('add-passkey-btn') as HTMLElement).onclick = async () => {
            try {
                const options = await apiRequest('GET', '/api/auth/passkey/register-options');
                const attResp = await (window as any).SimpleWebAuthnBrowser.startRegistration(options);
...
function bindEvents() {
    ...
    if (document.getElementById('avatar-upload-container') && !uploadWidget) {
        ...
        (document.querySelector('main') as HTMLElement).addEventListener('click', (e) => {
            const container = (e.target as HTMLElement).closest('#profile-picture-container');
            if (container) {
    ...
    // 2FA Management
    (document.getElementById('manage-passkeys-btn') as HTMLElement).onclick = () => managePasskeys();

    // Safety Info Inline Editor Toggle
    const displayDiv = document.getElementById('safety-info-display') as HTMLElement;
    const formDiv = document.getElementById('safety-info-form') as HTMLElement;
    const editBtn = document.getElementById('edit-safety-btn') as HTMLElement;
    const cancelBtn = document.getElementById('cancel-safety-btn') as HTMLElement;
    ...
    (formDiv as HTMLFormElement).onsubmit = async (e) => {
        e.preventDefault();
        const updateData = {
            first_aid_expiry: (document.getElementById('input-first-aid') as HTMLInputElement).value,
            phone_number: (document.getElementById('input-emergency') as HTMLInputElement).value
        };
        try {
    ...
    (document.getElementById('change-password-btn') as HTMLElement).onclick = async () => {
    ...
    (document.getElementById('delete-account-btn') as HTMLElement).onclick = async () => {
    ...
    (document.getElementById('sidebar-logout-btn') as HTMLElement).onclick = async () => {
