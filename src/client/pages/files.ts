...
async function loadCategories(): Promise<void> {
    ...
    try {
        ...
        filter.innerHTML = '<option value="">All Categories</option>' +
            categories.map((c: { id: string; name: string }) => `<option value="${c.id}">${c.name}</option>`).join('');
    } catch (e) {
...
async function fetchFiles(): Promise<void> {
    ...
    thead.innerHTML = `<tr>${columns.map((c: { key: string; label: string; sort: string }) => `
    ...
    try {
        ...
        list.innerHTML = files.map((file: { id: string; filename: string; title: string; category_name: string; author: string; date: string; size: number }) => {
...
// Router subscription
ViewChangedEvent.subscribe(async ({ viewId }: any) => {
    if (viewId === 'files') {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('input', (e: Event) => {
        ...
        if (target.id === 'file-search') {
            currentOptions.search = (target as HTMLInputElement).value;
    ...
    document.addEventListener('change', (e: Event) => {
        ...
        if (target.id === 'category-filter') {
            currentOptions.categoryId = (target as HTMLSelectElement).value;
            currentOptions.page = 1;
            fetchFiles();
        }
    });
