...
import { setupNumberInput, downloadCSV } from '@/utils/utils';
import { Panel } from '@/widgets/panel';
import { Modal } from '@/widgets/Modal';
import { Pagination } from '@/widgets/Pagination';
import { CURRENCY_POUND_SVG, ADD_SVG, TRIP_SVG, CHECK_SVG, CLOSE_SVG, EDIT_SVG, GROUP_SVG, WALLET_SVG, DELETE_SVG } from '@/utils/icons';
...
interface Driver {
    ...
    start_mileage_proof_id?: number;
    end_mileage_proof_id?: number;
    user_id: number;
    first_name: string;
    last_name: string;
}
...
export async function renderFinanceTab(container: HTMLElement, event: EventData, attendees: any[], userPerms: string[]) {
    container.innerHTML = `
    ...
    const attendeesContainer = container.querySelector('#admin-attendees-container') as HTMLElement;
    const transportContainer = container.querySelector('#admin-transport-container') as HTMLElement | null;
    const expensesContainer = container.querySelector('#admin-expenses-container') as HTMLElement;
    const summarySection = container.querySelector('#finance-summary-section') as HTMLElement;

    let attendeePage = 1;
    const attendeePageSize = 5;
    let attendeeFilter = '';
    let attendeesListFull: Attendee[] = [];
    ...
    const loadAllData = async () => {
        try {
            ...
            const summary: Summary = summaryRes.data || { breakdown: [] };
            ...
            const renderAttendees = () => {
                ...
                const filterInput = attendeesContainer.querySelector('#attendee-filter-input') as HTMLInputElement;
                filterInput.oninput = (e: Event) => {
                    attendeeFilter = (e.target as HTMLInputElement).value;
                    renderAttendees();
                    // Focus back since re-render loses it
                    const newInput = attendeesContainer.querySelector('#attendee-filter-input') as HTMLInputElement;
                    newInput.focus();
                    newInput.setSelectionRange(attendeeFilter.length, attendeeFilter.length);
                };

                const addBtn = attendeesContainer.querySelector('#admin-add-participant-btn') as HTMLElement;
                if (addBtn) addBtn.onclick = () => openAddParticipantModal(event.id, loadAllData);

                const pagerContainer = attendeesContainer.querySelector('#attendees-pagination') as HTMLElement;
                if (pagerContainer) {
                ...
                attendeesContainer.querySelectorAll('[data-remove-attendee]').forEach(btn => {
                    (btn as HTMLElement).onclick = async () => {
                        ...
                        try {
                            await apiRequest('DELETE', `/api/admin/events/${event.id}/attendees/${(btn as HTMLElement).dataset.removeAttendee}`);
                            notify('Success', 'Removed.', 'success', 5000, 'admin-attendee'); loadAllData();
                        } catch (e: any) { notify('Error', e.message, 'error', 5000, 'admin-attendee'); }
                    };
                });
                ...
                attendeesContainer.querySelectorAll('[data-refund-upfront]').forEach(btn => {
                    (btn as HTMLElement).onclick = async () => {
                        ...
                        try {
                            await apiRequest('POST', `/api/admin/events/${event.id}/attendees/${(btn as HTMLElement).dataset.refundUpfront}/refund-upfront`);
            ...
            if (transportContainer) {
                ...
                const addTripBtn = transportContainer.querySelector('#admin-add-trip-btn') as HTMLElement;
                ...
                transportContainer.querySelectorAll('[data-manage-trip-drivers]').forEach(btn => {
                    (btn as HTMLElement).onclick = () => openManageDriversModal(event.id, parseInt((btn as HTMLElement).dataset.manageTripDrivers!), attendeesListFull, loadAllData, event.costs_released);
                });
                transportContainer.querySelectorAll('[data-manage-trip-exclusions]').forEach(btn => {
                    (btn as HTMLElement).onclick = () => openExclusionsModal('trip', parseInt((btn as HTMLElement).dataset.manageTripExclusions!), attendeesListFull, loadAllData);
                });
            }
            ...
            const addExpenseBtn = expensesContainer.querySelector('#admin-add-expense-btn') as HTMLElement;
            ...
            expensesContainer.querySelectorAll('[data-manage-expense-exclusions]').forEach(btn => {
                (btn as HTMLElement).onclick = () => openExclusionsModal('expense', parseInt((btn as HTMLElement).dataset.manageExpenseExclusions!), attendeesListFull, loadAllData);
            });
            ...
            if (hasCosts) {
                ...
                const costItems = [
                    ...(summary.trips || []).map(t => ({
                        ...
                        contributions: t.drivers.reduce((acc: any, d) => { acc[d.user_id] = d.reimbursement; return acc; }, {})
                    })),
                ...
                const matrixRowsHtml = (summary.breakdown || []).map(row => {
                    const cells = costItems.map(item => {
                        ...
                        const contribution = (item.contributions as any)[row.id] || 0;
                ...
                const generateSettlementCSV = () => {
                    ...
                    (summary.trips || []).forEach(trip => {
                        ...
                        trip.drivers.forEach(d => csvData.push([d.name, d.miles.toString(), d.reimbursement.toFixed(2)]));
                        csvData.push([]);
                    });
                ...
                (summarySection.querySelector('#download-settlement-csv-btn') as HTMLElement).onclick = generateSettlementCSV;

                const releaseBtn = summarySection.querySelector('#release-funds-btn') as HTMLElement;
                if (releaseBtn) {
                    releaseBtn.onclick = async () => {
                        ...
                        } catch (e: any) { notify('Error', e.message, 'error', 5000, 'admin-finance'); }
                    };
                }
...
function openAddParticipantModal(eventId: number, onSuccess: () => void) {
    const modalContent = `
    ...
    const searchInput = document.getElementById('add-attendee-search-input') as HTMLInputElement;
    const resultsDropdown = document.getElementById('add-attendee-results-dropdown') as HTMLElement;
        ...
        searchTimeout = setTimeout(async () => {
            ...
            try {
                ...
                if (users.length === 0) resultsDropdown.innerHTML = '<p class="small-text p-3">No members found.</p>';
                    ...
                    resultsDropdown.innerHTML = users.map((u: any) => `
                    ...
                    resultsDropdown.querySelectorAll('.search-result-item').forEach(item => {
                        (item as HTMLElement).onclick = async () => {
                            ...
                            try {
                                await apiRequest('POST', `/api/admin/events/${eventId}/attendees`, { userId: (item as HTMLElement).dataset.userId });
...
function openAddTripModal(eventId: number, onSuccess: () => void) {
    ...
    document.body.insertAdjacentHTML('beforeend', modal.getHTML());
    modal.attachListeners(); modal.show();
    (document.getElementById('add-trip-form') as HTMLFormElement).onsubmit = async (e) => {
        e.preventDefault();
        try {
            await apiRequest('POST', `/api/admin/events/${eventId}/trips`, { name: (document.getElementById('trip-name') as HTMLInputElement).value });
            notify('Success', 'Trip created.', 'success', 5000, 'admin-transport'); modal.close(); onSuccess();
        } catch (err: any) { notify('Error', err.message, 'error', 5000, 'admin-transport'); }
    };
}

async function openManageDriversModal(eventId: number, tripId: number, attendees: Attendee[], onSuccess: () => void, isReadOnly = false) {
    const response = await apiRequest('GET', `/api/admin/events/${eventId}/drivers`);
    const drivers: Driver[] = (response.data || []).filter((d: Driver) => d.trip_id == tripId);
    ...
    if (modal.element) {
        const addDriverBtn = modal.element.querySelector('#admin-add-driver-btn') as HTMLElement;
        ...
        modal.element.querySelectorAll('[data-driver-status]').forEach(btn => {
            (btn as HTMLElement).onclick = async () => {
                try {
                    await apiRequest('POST', `/api/admin/drivers/${(btn as HTMLElement).dataset.id}/status`, { status: (btn as HTMLElement).dataset.driverStatus });
                    notify('Success', `Driver marked as ${(btn as HTMLElement).dataset.driverStatus}.`, 'success', 5000, 'admin-transport');
                    modal.close(); openManageDriversModal(eventId, tripId, attendees, onSuccess, isReadOnly); onSuccess();
                } catch (e: any) { notify('Error', e.message, 'error', 5000, 'admin-transport'); }
            };
        });
        ...
        modal.element.querySelectorAll('[data-remove-driver]').forEach(btn => {
            (btn as HTMLElement).onclick = async () => {
                ...
                try {
                    await apiRequest('DELETE', `/api/admin/drivers/${(btn as HTMLElement).dataset.removeDriver}`);
        ...
        modal.element.querySelectorAll('[data-edit-mileage]').forEach(btn => {
            (btn as HTMLElement).onclick = () => {
                const driver = drivers.find(d => d.id == parseInt((btn as HTMLElement).dataset.editMileage!));
...
function openAddDriverModal(tripId: number, attendees: Attendee[], onSuccess: () => void) {
    const modalContent = `
    ...
    const userSelect = document.getElementById('driver-user-id') as HTMLInputElement;
    const carSelect = document.getElementById('driver-car-id') as HTMLInputElement;
    const submitBtn = document.getElementById('add-driver-submit') as HTMLInputElement;
    ...
    userSelect.onchange = async () => {
        ...
        try {
            ...
            carSelect.innerHTML = cars.map((c: any) => `<option value="${c.id}">${c.name} (${c.seats}S / ${c.boats}B)</option>`).join('') || '<option value="" disabled>No cars found for user</option>';
    ...
    (document.getElementById('add-driver-form') as HTMLFormElement).onsubmit = async (e) => {
        e.preventDefault();
        try {
...
function openEditMileageModal(driver: Driver, onSuccess: () => void) {
    const modalContent = `
    ...
    const startMileageInput = document.getElementById('edit-start-mileage') as HTMLInputElement | null;
    const endMileageInput = document.getElementById('edit-end-mileage') as HTMLInputElement | null;
    if (startMileageInput) setupNumberInput(startMileageInput);
    if (endMileageInput) setupNumberInput(endMileageInput);

    (document.getElementById('edit-mileage-form') as HTMLFormElement).onsubmit = async (e) => {
        e.preventDefault();
        const start = (document.getElementById('edit-start-mileage') as HTMLInputElement).value;
        const end = (document.getElementById('edit-end-mileage') as HTMLInputElement).value;
        try {
...
function openAddExpenseModal(eventId: number, attendees: Attendee[], onSuccess: () => void) {
    const modalContent = `
    ...
    const amountInput = document.getElementById('expense-amount') as HTMLInputElement | null;
    if (amountInput) setupNumberInput(amountInput);

    (document.getElementById('add-expense-form') as HTMLFormElement).onsubmit = async (e) => {
        e.preventDefault();
        const data = {
            userId: (document.getElementById('expense-user-id') as HTMLInputElement).value,
            amount: (document.getElementById('expense-amount') as HTMLInputElement).value,
            description: (document.getElementById('expense-description') as HTMLInputElement).value
        };
        try {
        ...
        } catch (err: any) { notify('Error', err.message, 'error', 5000, 'admin-finance'); }
    };
}

export async function openExclusionsModal(type: string, id: number, attendees: Attendee[], onSuccess: () => void) {
    const currentExclusions = await apiRequest('GET', `/api/admin/${type}s/${id}/exclusions`);
    const excludedIds: number[] = currentExclusions.data || [];
    ...
    (document.getElementById('exclusions-form') as HTMLFormElement).onsubmit = async (e) => {
        e.preventDefault();
        const userIds = Array.from(new FormData(e.target as HTMLFormElement).getAll('userIds')).map(id => parseInt(id as string));
        try {
