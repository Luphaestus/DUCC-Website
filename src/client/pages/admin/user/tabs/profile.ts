...
import { showPasswordModal } from '@/utils/modal';
import { MembershipChangedEvent } from '@/utils/events/events';
import { renderUserDetail } from '../detail.js';
import { getOrdinal, setupNumberInput } from '@/utils/utils';
import { Panel } from '@/widgets/panel';
import { ValueHeader } from '@/widgets/value_header';
import { POOL_SVG, ADD_SVG, PERSON_SVG, EDIT_SVG, BOLT_SVG, ID_CARD_SVG, SHIELD_SVG, CLOSE_SVG } from '@/utils/icons';

/**
 * Main rendering and logic binding function for the Admin Profile tab.
 * 
 * @param {HTMLElement} container - Tab content area.
 * @param {any} user - Target user data object.
 ...
 */
export async function renderProfileTab(container: HTMLElement, user: any, userPerms: string[], canManageUsers: boolean, isExec: boolean): Promise<void> {
    ...
    const colleges: any[] = collegesRes || [];
                    ...
                    content: `
                        <div class="card-body">
                            ...
                            <div id="direct-perms-list" class="tags-cloud">
                                ${(user.direct_permissions || []).map((p: any) => `
    ...
    if (canManageUsers) {
        const roleSelect = document.getElementById('admin-user-role-select') as HTMLSelectElement | null;
        const addRoleBtn = document.getElementById('add-role-btn');
        
        if (roleSelect && addRoleBtn) {
            const allRoles: any[] = await apiRequest('GET', '/api/admin/roles').catch(() => []);
            ...
            container.querySelectorAll('.remove-role-btn').forEach(btn => {
                (btn as HTMLElement).onclick = async () => {
                    try {
                        await apiRequest('DELETE', `/api/admin/user/${user.id}/role/${(btn as HTMLElement).dataset.id}`);
        ...
        const instructorToggle = document.getElementById('admin-user-instructor') as HTMLInputElement | null;
        if (instructorToggle) {
        ...
        const permSelect = document.getElementById('add-perm-select') as HTMLSelectElement | null;
        if (permSelect) {
            const allPerms: any[] = await apiRequest('GET', '/api/admin/roles/permissions').catch(() => []);
            ...
            container.querySelectorAll('.remove-perm-btn').forEach(btn => {
                (btn as HTMLElement).onclick = async () => {
                    try {
                        await apiRequest('DELETE', `/api/admin/user/${user.id}/permission/${(btn as HTMLElement).dataset.id}`);
        ...
        if (editBtn) {
            ...
            const form = document.getElementById('account-info-form') as HTMLFormElement | null;
            const cancelBtn = document.getElementById('cancel-account-btn');

            if (form && displayDiv && cancelBtn) {
                ...
                form.onsubmit = async (e) => {
                    e.preventDefault();
                    let emailVal = (document.getElementById('input-email') as HTMLInputElement).value;
                    if (emailVal && !emailVal.includes('@')) emailVal += '@durham.ac.uk';

                    const updateData: any = {
                        email: emailVal,
                        phone_number: (document.getElementById('input-phone') as HTMLInputElement).value,
                        college_id: parseInt((document.getElementById('input-college') as HTMLSelectElement).value) || null
                    };
                    if (canManageUsers) {
                        updateData.free_sessions = parseInt((document.getElementById('input-free-sessions') as HTMLInputElement).value);
                        updateData.swims = parseInt((document.getElementById('input-total-swims') as HTMLInputElement).value);
                        updateData.is_member = (document.getElementById('input-is-member') as HTMLInputElement).checked;
                        updateData.is_instructor = (document.getElementById('input-is-instructor') as HTMLInputElement).checked;
                    }
                    try {
        ...
        if (balanceCard) {
            balanceCard.onclick = () => {
                const txTab = document.querySelector('button[data-tab="transactions"]') as HTMLElement | null;
        ...
        if (canManageSwims) {
            ...
            const addBootieBtn = document.getElementById('admin-add-bootie-btn');
            if (addBootieBtn) {
                addBootieBtn.onclick = async () => {
                    try {
        ...
        const difficultyInput = document.getElementById('admin-user-difficulty') as HTMLInputElement | null;
        if (difficultyInput) {
