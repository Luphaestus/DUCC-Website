...
import { SAVE_SVG, IMAGE_SVG, UPLOAD_SVG, CLOSE_SVG } from '@/utils/icons';
import { Modal } from '@/widgets/Modal';

let imagePickerModal: Modal | null = null;
let modalUploadWidget: UploadWidget | null = null;

/**
 * Main rendering function for the globals management dashboard.
 */
export async function renderManageGlobals(): Promise<void> {
...
let activePickerKey: string | null = null;

async function selectImage(url: string): Promise<void> {
    if (!activePickerKey) return;

    const input = document.querySelector(`.global-input[data-key="${activePickerKey}"]`) as HTMLInputElement | null;
    if (input) {
        input.value = url;
 ...
 */
async function fetchAndRenderGlobals(): Promise<void> {
    const tbody = document.getElementById('globals-table-body');
    if (!tbody) return;

    try {
        const globalsRes = await apiRequest('GET', '/api/globals');
        const globals = globalsRes.res || {};
        ...
        tbody.innerHTML = Object.entries(globals).map(([key, value]: [string, any]) => {
        ...
        tbody.querySelectorAll('.picker-btn').forEach(btn => {
            (btn as HTMLElement).onclick = () => {
                activePickerKey = (btn as HTMLElement).dataset.key || null;
                if (imagePickerModal && activePickerKey) {
                    imagePickerModal.show();

                    // Provide current value as preview
                    const currentVal = (tbody.querySelector(`.global-input[data-key="${activePickerKey}"]`) as HTMLInputElement)?.value;
        ...
        tbody.querySelectorAll('.image-preview-global').forEach(el => {
            (el as HTMLElement).onclick = (e: MouseEvent) => {
                e.stopPropagation();
                const wasVisible = (el as HTMLElement).classList.contains('preview-open');

                document.querySelectorAll('.image-preview-global.preview-open').forEach(p => p.classList.remove('preview-open'));

                if (!wasVisible) {
                    (el as HTMLElement).classList.add('preview-open');
                }
            };
        });
        ...
        for (const [key] of Object.entries(globals)) {
            ...
            if (btn) {
                btn.addEventListener('click', async () => {
                    const input = tbody.querySelector(`.global-input[data-key="${key}"]`) as HTMLInputElement;
                    if (input) {
                        const newValue = input.value;
                        const displayName = (globals as any)[key]?.name || key;
 ...
 * @param {string} displayName - Friendly name for notifications.
 */
async function updateGlobal(key: string, value: string, displayName: string): Promise<void> {
    let parsedValue: any = isNaN(value as any) || value.trim() === '' ? value : parseFloat(value);
    ...
    await apiRequest('POST', `/api/globals/${key}`, payload).then(() => {
        ...
        notify(`Failed to Update ${displayName}`, e.message || e, 'error');
    });
}