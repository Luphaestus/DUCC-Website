...
import { BalanceChangedEvent, EventAttendanceChangedEvent } from '@/utils/events/events';
...
import { initTooltips } from '@/widgets/tooltip';
 ...
 */
function renderUserBubbles(users: any[], canManage: boolean, checkLeftStatus = true): string {
 ...
 */
async function fillAttendeesList(eventId: string | number, canManage: boolean): Promise<void> {
    try {
 ...
 */
async function fillWaitlist(eventId: number | string, isFull: boolean, canManage: boolean): Promise<void> {
    try {
 ...
 * @param {boolean} canManage
 */
async function setupEventButtons(eventId: number | string, path: string, resolvedPath: string, canManage: boolean): Promise<void> {
    try {
        const loggedInRes = await apiRequest('GET', '/api/auth/status').catch(() => ({ authenticated: false }));
        const loggedIn = loggedInRes.authenticated;
        const attendButton = document.getElementById('attend-event-button') as HTMLButtonElement | null;
        ...
        const attendees: any[] = attendeesResponse?.attendees || [];
        ...
        if (attendButton) {
            ...
            const newBtn = attendButton.cloneNode(true) as HTMLButtonElement;
            if (attendButton.parentNode) attendButton.parentNode.replaceChild(newBtn, attendButton);
            ...
            if (!newBtn.classList.contains('hidden')) {
                newBtn.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        notify('Action Failed', error.message || error, 'error', 5000, 'event-action');
                    }
                });
            }
        }
 ...
 */
async function NavigationEventListner({ viewId, path, resolvedPath }: any): Promise<void> {
    if (viewId !== "event") {
        const ev = document.getElementById('event-view');
    ...
    try {
        ...
        const tagsHtml = (event.tags || []).map((tag: any) => Tag.render(tag, '', `--tag-colour: ${tag.color}65;`)).join('');
        ...
        if (event.upfront_cost > 0) {
            ...
            const formatOptions: Intl.DateTimeFormatOptions = { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit', hour12: true };
        ...
        const acceptedTrips = (Array.isArray(event.driver_info) ? event.driver_info : (event.driver_info ? [event.driver_info] : []))
            .filter((info: any) => info.status === 'accepted');
        ...
        if (canReport && !isCanceled) {
            if (acceptedTrips.length === 1) {
                ...
                const tripPanels = acceptedTrips.map((info: any) => {
        ...
        if (canManage) {
            const editBtn = document.getElementById('edit-event-button') as HTMLButtonElement | null;
            if (editBtn) {
 ...
 */
function setupExpensePanel(eventId: string | number): void {
    ...
    let receiptFileId: string | number | null = null;
    const uploadWidget = new UploadWidget('expense-upload-widget', {
        ...
        onUploadComplete: (result: string | number | number[]) => {
            if (Array.isArray(result)) receiptFileId = result[0];
            else receiptFileId = result;
            if (uploadWidget.widgetEl) uploadWidget.widgetEl.style.borderColor = '';
        },
        onRemove: () => {
            receiptFileId = null;
            return true;
    ...
    const submitBtn = document.getElementById('submit-expense-btn') as HTMLButtonElement | null;
    const descInput = document.getElementById('expense-description') as HTMLInputElement | null;
    ...
    if (submitBtn) {
        submitBtn.addEventListener('click', async () => {
            ...
            if (!receiptFileId) {
                if (uploadWidget.widgetEl) uploadWidget.widgetEl.style.borderColor = 'var(--colour-bad)';
                hasError = true;
            } else {
                if (uploadWidget.widgetEl) uploadWidget.widgetEl.style.borderColor = '';
            }
            ...
            try {
                ...
                const anotherBtn = document.getElementById('submit-another-expense-btn');
                if (anotherBtn) anotherBtn.onclick = () => setupExpensePanel(eventId);
            } catch (e: any) {
...
if (mainEl) {
    mainEl.addEventListener('click', (e: MouseEvent) => {
        const target = (e.target as HTMLElement).closest('.attendee-bubble[data-user-id]') as HTMLElement | null;
        if (target) {
            const ev = document.getElementById('event-view');
            if (ev && ev.contains(target)) {
