...
// Add to window interface for global callback
declare global {
    interface Window {
        editExecMember: (id: number) => void;
    }
}

let currentUserPerms: string[] = [];

async function fetchAndRenderExec(): Promise<void> {
    ...
    if (!grid || !pastContainer || !adminActions) return;
    ...
    try {
        ...
        const execData = execRes as ExecData;
        ...
        if (canManage) {
            ...
            const addBtn = document.getElementById('add-exec-btn');
            if (addBtn) addBtn.onclick = () => openExecModal();
        }
        ...
        if (execData.current.length === 0) {
            ...
            grid.innerHTML = Object.keys(grouped).map(Number).sort((a, b) => a - b).map(rank => `
        ...
        if (execData.past.length > 0) {
            const pastSection = document.getElementById('past-exec-section');
            ...
            pastContainer.innerHTML = Object.keys(grouped).map(Number).sort((a, b) => b - a).map(year => `
                    ...
                    <div class="past-exec-list">
                        ${grouped[year].map(m => `
                            <div class="past-member-row">
                                ...
                                ${canManage ? `
                                    <button class="past-edit-btn" onclick="window.editExecMember(${m.id})" title="Edit Record">
                                        ${EDIT_SVG}
                                    </button>
                                ` : ''}
                            </div>
        ...
        // Bind Edit buttons
        window.editExecMember = (id: number) => {
            const member = [...execData.current, ...execData.past].find(m => m.id == id);
            if (member) openExecModal(member);
        };

        grid.querySelectorAll('[data-edit-exec]').forEach(btn => {
            (btn as HTMLElement).onclick = () => {
                const id = (btn as HTMLElement).dataset.editExec;
                const member = execData.current.find(m => m.id == Number(id));
                if (member) openExecModal(member);
            };
        });
        ...
        grid.querySelectorAll('[data-delete-exec]').forEach(btn => {
            (btn as HTMLElement).onclick = async () => {
                const id = (btn as HTMLElement).dataset.deleteExec;
        ...
        if (grid) grid.innerHTML = `<p class="error-text">Failed to load committee: ${err.message}</p>`;
    }
}

function renderExecCard(member: ExecMember, canManage: boolean): string {
...
function openExecModal(member: ExecMember | null = null): void {
    const isEdit = !!member;
    ...
    const ppInput = document.getElementById('exec-pp-override') as HTMLInputElement;
    new UploadWidget('exec-pp-upload-container', {
        mode: 'inline',
        defaultPreview: (member?.profile_picture_path && member?.profile_picture_override_id) ? member.profile_picture_path : null,
        onUploadComplete: (result: string | number | number[]) => {
            if (ppInput) ppInput.value = String(Array.isArray(result) ? result[0] : result);
        },
        onRemove: () => {
            if (ppInput) ppInput.value = '';
            return true;
        },
        onImageSelect: (data: any) => {
            if (ppInput) ppInput.value = data.id ? String(data.id) : '';
        }
    });

    // User Search Logic
    const searchInput = document.getElementById('exec-user-search') as HTMLInputElement;
    const resultsDropdown = document.getElementById('exec-user-results') as HTMLElement;
    const userIdInput = document.getElementById('exec-user-id') as HTMLInputElement;
    const firstNameInput = document.getElementById('exec-first-name-override') as HTMLInputElement;
    const lastNameInput = document.getElementById('exec-last-name-override') as HTMLInputElement;
    const emailInput = document.getElementById('exec-email-override') as HTMLInputElement;
                ...
                try {
                    ...
                    if (resultsDropdown) {
                        if (users.length === 0) {
                            ...
                            resultsDropdown.innerHTML = (users as any[]).map(u => `
                            ...
                            resultsDropdown.querySelectorAll('.search-result-item').forEach(item => {
                                (item as HTMLElement).onclick = () => {
                                    const u = JSON.parse((item as HTMLElement).dataset.user!);
                                    if (userIdInput) userIdInput.value = u.id;
                                    // Optionally autofill overrides if they are empty
                                    if (firstNameInput && !firstNameInput.value) firstNameInput.value = u.first_name;
                                    if (lastNameInput && !lastNameInput.value) lastNameInput.value = u.last_name;
    ...
    const outsideClickListener = (e: MouseEvent) => {
        ...
        if (searchInput && resultsDropdown && !searchInput.contains(target) && !resultsDropdown.contains(target)) {
            resultsDropdown.classList.add('hidden');
        }
    };
    ...
    if (execForm) {
        execForm.onsubmit = async (e) => {
            ...
            const data = {
                ...
                roleName: (document.getElementById('exec-role-name') as HTMLInputElement).value,
                displayOrder: parseInt((document.getElementById('exec-display-order') as HTMLInputElement).value),
                isCurrent: (document.getElementById('exec-is-current') as HTMLInputElement).checked ? 1 : 0,
                termStart: (document.getElementById('exec-term-start') as HTMLInputElement).value || null,
                termEnd: (document.getElementById('exec-term-end') as HTMLInputElement).value || null,
...
if (mainElement) mainElement.insertAdjacentHTML('beforeend', HTML_TEMPLATE);