...
import { EventAttendanceChangedEvent } from '@/utils/events/events';
 ...
 */
async function NavigationEventListner({ viewId, path }: any): Promise<void> {
...
async function renderExpenseView(path: string): Promise<void> {
    ...
    if (isEdit) {
        ...
        try {
            ...
            expense = (res.data || []).find((e: any) => e.id == expenseId);
    ...
    let uploadedFileId: string | number | null = expense ? expense.receipt_file_id : null;
    ...
    const amountInput = document.getElementById('expense-view-amount') as HTMLInputElement | null;
    if (amountInput) setupNumberInput(amountInput);

    const widgetContainer = container.querySelector('#expense-view-upload-container') as HTMLElement | null;
    if (widgetContainer) {
        new UploadWidget(widgetContainer, {
            mode: 'inline',
            selectMode: 'single',
            autoUpload: true,
    ...
    const expenseForm = document.getElementById('expense-view-form') as HTMLFormElement | null;
    if (expenseForm) {
        expenseForm.onsubmit = async (e) => {
            e.preventDefault();
            const amountVal = (document.getElementById('expense-view-amount') as HTMLInputElement).value;
            const descVal = (document.getElementById('expense-view-desc') as HTMLInputElement).value;
...
async function renderMileageView(path: string): Promise<void> {
    ...
    let currentMileage: string | number = '';
    let uploadedFileId: string | number | null = null;
    ...
    const mileageInput = document.getElementById('mileage-view-input') as HTMLInputElement | null;
    if (mileageInput) setupNumberInput(mileageInput);

    const widgetContainer = container.querySelector('#mileage-view-upload-container') as HTMLElement | null;
    if (widgetContainer) {
        new UploadWidget(widgetContainer, {
            mode: 'inline',
            selectMode: 'single',
    ...
    const mileageForm = document.getElementById('mileage-view-form') as HTMLFormElement | null;
    if (mileageForm) {
        mileageForm.onsubmit = async (e) => {
            e.preventDefault();
            if (!uploadedFileId) return notify('Error', 'Please upload a photo of your odometer.', 'error');

            try {
                const mileageVal = (document.getElementById('mileage-view-input') as HTMLInputElement).value;
