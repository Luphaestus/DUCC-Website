...
import { addRoute, switchView, ViewChangedEvent } from '@/utils/view';
import { BOLT_SVG } from '@/utils/icons';
...
function ViewNavigationEventListener({ resolvedPath }: { resolvedPath: string }) {
    if (resolvedPath === '/set-password') {
        ...
        const tokenInput = document.getElementById('set-password-token') as HTMLInputElement;
        const form = document.getElementById('set-password-form') as HTMLFormElement;

        if (token) {
            if (tokenInput) tokenInput.value = token;
            if (form) (form.querySelector('button') as HTMLButtonElement).disabled = false;

        } else {
            if (form) (form.querySelector('button') as HTMLButtonElement).disabled = true;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const newPassowordInput = document.getElementById('new-password') as HTMLInputElement;
        const newPassword = newPassowordInput.value;
        const confirmPasswordInput = document.getElementById('confirm-password') as HTMLInputElement;
        const confirmPassword = confirmPasswordInput.value;
        const submitButton = document.getElementById('set-password-submit') as HTMLButtonElement;
