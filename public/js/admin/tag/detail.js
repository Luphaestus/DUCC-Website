...
export async function renderTagDetail(id) {
    ...
    if (!isNew) {
        try {
            const tags = await ajaxGet('/api/tags');
            // API returns array of tags
            tag = tags.find(t => t.id == id);
            if (!tag) throw new Error('Tag not found');
            
            whitelist = await ajaxGet(`/api/tags/${id}/whitelist`);
        } catch (e) {
            console.error(e);
    ...
    if (!isNew) {
        ...
        document.getElementById('whitelist-form').onsubmit = async (e) => {
            ...
            try {
                ...
                const newWhitelist = await ajaxGet(`/api/tags/${id}/whitelist`);
    ...
    tbody.onclick = async (e) => {
        if (e.target.classList.contains('remove-whitelist-btn')) {
            ...
            if (confirm('Remove user from whitelist?')) {
                 ...
                 const newWhitelist = await ajaxGet(`/api/tags/${tagId}/whitelist`);
