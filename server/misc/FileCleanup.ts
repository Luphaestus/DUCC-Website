...
export default class FileCleanup {
     ...
     */
    static async checkAndDeleteIfUnused(db: DatabaseWrapper, fileIdOrUrl: string | number): Promise<void> {
        ...
        try {
            ...
            if (defaultImage && typeof defaultImage === 'string' && defaultImage.includes(`/api/files/${fileId}/`)) return;
