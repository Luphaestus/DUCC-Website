...
export default class AdminExpensesAPI {
    ...
    registerRoutes() {
        ...
        this.app.post('/api/admin/events/:eventId/trips', checkAuthentication('event.manage.all|event.manage.scoped'), async (req, res) => {
            const { name } = req.body;
        ...
        this.app.post('/api/admin/trips/:id/exclusions', checkAuthentication('event.manage.all|event.manage.scoped'), async (req, res) => {
        ...
        this.app.get('/api/admin/trips/:id/exclusions', checkAuthentication('event.manage.all|event.manage.scoped'), async (req, res) => {
            const status = await ExpensesDB.getExclusions(this.db, 'trip', req.params.id);
            status.getResponse(res);
        });

        /** Set exclusions for an expense. */
        this.app.post('/api/admin/expenses/:id/exclusions', checkAuthentication('event.manage.all|event.manage.scoped'), async (req, res) => {
            const { userIds } = req.body;
        ...
        this.app.get('/api/admin/expenses/:id/exclusions', checkAuthentication('event.manage.all|event.manage.scoped'), async (req, res) => {
            const status = await ExpensesDB.getExclusions(this.db, 'expense', req.params.id);
            status.getResponse(res);
        });

        /** Get financial summary/preview for an event. */
        this.app.get('/api/admin/events/:eventId/finance-summary', checkAuthentication('event.manage.all|event.manage.scoped'), async (req, res) => {
            const status = await ExpensesDB.getFinanceSummary(this.db, req.params.eventId);
            status.getResponse(res);
        });

        /** Release event costs. */
        this.app.post('/api/admin/events/:eventId/release-costs', checkAuthentication('event.manage.all|event.manage.scoped'), async (req, res) => {
            const status = await ExpensesDB.releaseEventCosts(this.db, req.params.eventId);
            status.getResponse(res);
        });

        /** Refund upfront fee for an attendee. */
        this.app.post('/api/admin/events/:eventId/attendees/:userId/refund-upfront', checkAuthentication('transaction.manage|event.manage.all'), async (req, res) => {
            const status = await ExpensesDB.refundUpfrontFee(this.db, req.params.eventId, req.params.userId);
            status.getResponse(res);
        });

        /** Remove an attendee from an event. */
        this.app.delete('/api/admin/events/:eventId/attendees/:userId', checkAuthentication('event.manage.all|event.manage.scoped'), async (req, res) => {
        ...
        this.app.post('/api/admin/events/:eventId/attendees', checkAuthentication('event.manage.all|event.manage.scoped'), async (req, res) => {
        ...
        this.app.post('/api/admin/events/:eventId/expenses', checkAuthentication('event.manage.all|event.manage.scoped'), async (req, res) => {
        ...
        this.app.post('/api/admin/trips/:tripId/drivers', checkAuthentication('event.manage.all|event.manage.scoped'), async (req, res) => {
        ...
        this.app.delete('/api/admin/drivers/:id', checkAuthentication('event.manage.all|event.manage.scoped'), async (req, res) => {
            const status = await CarsDB.removeDriver(this.db, req.params.id);
            status.getResponse(res);
        });

        /** Manually update driver mileage (Admin action). */
        this.app.post('/api/admin/drivers/:id/mileage', checkAuthentication('event.manage.all|event.manage.scoped'), async (req, res) => {
