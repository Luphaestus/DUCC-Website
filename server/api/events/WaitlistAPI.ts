...
export default class WaitlistAPI {
    ...
    registerRoutes() {
         ...
         */
        this.app.get('/api/event/:id/isOnWaitlist', check(), async (req: any, res: Response) => {
         ...
         */
        this.app.post('/api/event/:id/waitlist/join', check(), async (req: any, res: Response) => {
            const eventId = parseInt(req.params.id, 10);
            if (Number.isNaN(eventId)) return res.status(400).json({ message: 'Event ID must be an integer' });
         ...
         */
        this.app.post('/api/event/:id/waitlist/leave', check(), async (req: any, res: Response) => {
            const eventId = parseInt(req.params.id, 10);
            if (Number.isNaN(eventId)) return res.status(400).json({ message: 'Event ID must be an integer' });
         ...
         */
        this.app.get('/api/event/:id/waitlist', async (req: any, res: Response) => {
            const eventId = parseInt(req.params.id, 10);
