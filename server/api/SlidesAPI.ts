...
export default class SlidesAPI {
    ...
    registerRoutes() {
         ...
         */
        this.app.get('/api/slides/count', async (req: Request, res: Response) => {
         ...
         */
        this.app.get('/api/slides/images', async (req: Request, res: Response) => {
         ...
         */
        this.app.get('/api/slides/random', async (req: Request, res: Response) => {
            const status = await SlidesDB.getSlides(this.db);
            if (status.isError()) return status.getResponse(res);
         ...
         */
        this.app.post('/api/slides/import', check('file.write'), async (req: Request, res: Response) => {
         ...
         */
        this.app.delete('/api/slides', check('file.write'), async (req: Request, res: Response) => {
            const { fileId } = req.body;
            if (!fileId) return res.status(400).json({ message: 'fileId is required' });
