...
export default class ExecDB extends BaseDB {
     ...
     */
    static async getCurrentExec(db, includeHidden = false) {
     ...
     */
    static async syncExecMember(db, userId) {
        return this.wrap(async () => {
            ...
            const currentRoleNames = execRoles.map(r => r.name);
            ...
            await db.run(
                `UPDATE exec_committee ec
                 ...
                 SET ec.profile_picture_override_id = COALESCE(ec.profile_picture_override_id, u.profile_picture_id),
                     ...
                     ec.last_name_override = COALESCE(ec.last_name_override, u.last_name),
                     ec.email_override = COALESCE(ec.email_override, u.email)
                 WHERE ec.user_id = ? AND ec.is_current = 1 AND ec.role_name NOT IN (${currentRoleNames.map(() => '?').join(',')})`,
            ...
            await db.run(
                `UPDATE exec_committee 
                 ...
                 WHERE user_id = ? AND is_current = 1 AND role_name NOT IN (${currentRoleNames.map(() => '?').join(',')})`,
                [userId, ...currentRoleNames]
            );

            // Get the user's current profile picture to use as default override
            const user = await db.get('SELECT profile_picture_id FROM users WHERE id = ?', [userId]);
            const profilePictureId = user ? user.profile_picture_id : null;
     ...
     */
    static async addExecMember(db, { userId, roleName, displayOrder, termStart, termEnd, isCurrent = 1, firstNameOverride, lastNameOverride, emailOverride, profilePictureOverrideId, profilePictureColorOverride, profilePictureFontOverride, profilePictureInitialsOverride }) {
        return this.wrap(async () => {
    ...
    static async archiveCurrentCommittee(db) {
        ...
        if (!db.connection.getConnection) {
