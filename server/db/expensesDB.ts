...
interface ExpenseData {
    ...
    amount: number;
    description: string;
    receiptFileId: number | string | null;
}
...
export default class ExpensesDB extends BaseDB {
     ...
     */
    static async createTrip(db: DatabaseWrapper, eventId: number | string, name: string): Promise<statusObject> {
     ...
     */
    static async getTrips(db: DatabaseWrapper, eventId: number | string): Promise<statusObject> {
        return this.wrap(async () => {
     ...
     */
    static async createExpense(db: DatabaseWrapper, { eventId, userId, amount, description, receiptFileId }: ExpenseData): Promise<statusObject> {
        return this.wrap(async () => {
     ...
     */
    static async getExpenses(db: DatabaseWrapper, eventId: number | string): Promise<statusObject> {
        return this.wrap(async () => {
     ...
     * Update an expense.
     */
    static async updateExpense(db: DatabaseWrapper, id: number | string, { amount, description, receiptFileId }: Partial<ExpenseData>, userId: number | string | null = null): Promise<statusObject> {
        return this.wrap(async () => {
            ...
            const params: any[] = [amount, description, receiptFileId, id];
            if (userId) {
                query += ' AND user_id = ?';
                params.push(userId);
            }
     ...
     * Delete an expense.
     */
    static async deleteExpense(db: DatabaseWrapper, id: number | string, userId: number | string | null = null): Promise<statusObject> {
        return this.wrap(async () => {
            const event = await db.get(`
     ...
     */
    static async setExclusions(db: DatabaseWrapper, type: 'trip' | 'expense', id: number | string, userIds: (number | string)[]): Promise<statusObject> {
     ...
     */
    static async getExclusions(db: DatabaseWrapper, type: 'trip' | 'expense', id: number | string): Promise<statusObject> {
        const table = type === 'trip' ? 'trip_exclusions' : 'expense_exclusions';
        const idCol = type === 'trip' ? 'trip_id' : 'expense_id';
     ...
     */
    static async releaseEventCosts(db: DatabaseWrapper, eventId: number | string): Promise<statusObject> {
        return db.transaction(async (tx) => {
            ...
            const expenseExclusionMap: Record<number | string, any[]> = {};
            ...
            const tripExclusionMap: Record<number | string, any[]> = {};
            ...
            const userCostSheet: Record<number | string, number> = {};
            ...
            for (const [userId, amount] of Object.entries(userCostSheet)) {
                ...
                if (Math.abs(roundedAmount) > 0.001) {
                    // Using cast for TransactionsDB internal
                    await (TransactionsDB as any)._add_transaction_internal(tx, Number(userId), -roundedAmount, `Event Costs: ${event.title}`, eventId);
                }
            }
     ...
     */
    static async refundUpfrontFee(db: DatabaseWrapper, eventId: number | string, userId: number | string): Promise<statusObject> {
        return db.transaction(async (tx) => {
            ...
            await (TransactionsDB as any)._add_transaction_internal(tx, userId, refundAmount, `Upfront Fee Refund: ${event.title}`, eventId);
     ...
     */
    static async getFinanceSummary(db: DatabaseWrapper, eventId: number | string): Promise<statusObject> {
        return this.wrap(async () => {
            ...
            const breakdown: Record<number | string, any> = {};
            attendees.forEach(a => {
                breakdown[a.id] = {
                    id: a.id,
     ...
     */
    static async removeAttendee(db: DatabaseWrapper, eventId: number | string, userId: number | string): Promise<statusObject> {
        return this.wrap(async () => {
            const event = await db.get('SELECT costs_released FROM events WHERE id = ?', [eventId]);
            if (!event) return new statusObject(404, 'Event not found.');
     ...
     */
    static async addAttendee(db: DatabaseWrapper, eventId: number | string, userId: number | string): Promise<statusObject> {
        return this.wrap(async () => {
            const event = await db.get('SELECT costs_released FROM events WHERE id = ?', [eventId]);
            if (!event) return new statusObject(404, 'Event not found.');
