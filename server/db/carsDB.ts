...
export default class CarsDB {
     ...
     */
    static async getCars(db: DatabaseWrapper, userId: number | string | null = null, includeGlobal: boolean = true): Promise<statusObject> {
     ...
     * Create a new car.
     */
    static async createCar(db: DatabaseWrapper, { name, seats, boats, isGlobal = false, userId = null }: CarData): Promise<statusObject> {
        try {
     ...
     */
    static async updateCar(db: DatabaseWrapper, id: number | string, { name, seats, boats, isGlobal }: CarData, userId: number | string | null = null): Promise<statusObject> {
        try {
            let query = 'UPDATE cars SET name = ?, seats = ?, boats = ?, is_global = ? WHERE id = ?';
            const params: any[] = [name, seats, boats, isGlobal ? 1 : 0, id];
     ...
     * Delete a car.
     */
    static async deleteCar(db: DatabaseWrapper, id: number | string, userId: number | string | null = null): Promise<statusObject> {
        try {
     ...
     */
    static async assignDriver(db: DatabaseWrapper, tripId: number | string, userId: number | string, carId: number | string): Promise<statusObject> {
        try {
     ...
     */
    static async updateDriverStatus(db: DatabaseWrapper, id: number | string, status: string): Promise<statusObject> {
        try {
     ...
     */
    static async getEventDrivers(db: DatabaseWrapper, eventId: number | string): Promise<statusObject> {
        try {
     ...
     */
    static async getDriverById(db: DatabaseWrapper, id: number | string): Promise<statusObject> {
        try {
     ...
     */
    static async removeDriver(db: DatabaseWrapper, id: number | string, userId: number | string | null = null): Promise<statusObject> {
        try {
            const event = await db.get('SELECT costs_released FROM events e JOIN trips t ON e.id = t.event_id JOIN event_drivers ed ON t.id = ed.trip_id WHERE ed.id = ?', [id]);
     ...
     */
    static async submitMileage(db: DatabaseWrapper, id: number | string, type: 'start' | 'end', mileage: number, proofId: number | string): Promise<statusObject> {
        try {
