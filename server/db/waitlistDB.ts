...
export default class WaitlistDB {
     ...
     */
    static async is_user_on_waiting_list(db: DatabaseWrapper, userId: number, eventId: number): Promise<statusObject> {
     ...
     */
    static async join_waiting_list(db: DatabaseWrapper, userId: number, eventId: number): Promise<statusObject> {
     ...
     */
    static async leave_waiting_list(db: DatabaseWrapper, userId: number, eventId: number): Promise<statusObject> {
     ...
     */
    static async get_next_on_waiting_list(db: DatabaseWrapper, eventId: number): Promise<statusObject> {
     ...
     */
    static async remove_user_from_waiting_list(db: DatabaseWrapper, eventId: number, userId: number): Promise<void> {
        await db.run('DELETE FROM event_waiting_list WHERE event_id = ? AND user_id = ?', [eventId, userId]);
    }
     ...
     */
    static async get_waiting_list(db: DatabaseWrapper, eventId: number): Promise<statusObject> {
        try {
     ...
     */
    static async get_waiting_list_count(db: DatabaseWrapper, eventId: number): Promise<statusObject> {
        try {
     ...
     */
    static async get_waiting_list_position(db: DatabaseWrapper, eventId: number, userId: number): Promise<statusObject> {
        try {
